...
import {ChatDotRound, FolderOpened, Microphone, SwitchButton, VideoCamera} from "@element-plus/icons-vue";
...
const webSocket = new WebSocket(`ws://localhost:8080/singleChat`)
webSocket.onopen = () => {
...
const unReaded = async ()=>{
  ...
  consultants.value.forEach(consultant => {
    consultant.unReaded = res.data[consultant.cid]
  });
  users.value.forEach(user => {
    user.unReaded = res.data[user.uid]
  });
}
...
const getChatMessage = async (data, source) => {
  console.log(data)
  // 取消其他项的选中状态
  await consultants.value.forEach(consultant => {
    consultant.selected = false;
  });
  await users.value.forEach(user => {
    user.selected = false;
  });
      ...
      <el-aside>
        ...
        <div class="consultants-list-item"
          ...
          <template v-if="fromId !== consultant.cid">
            ...
            <div style="margin-right: 10px;margin-top: 20px">
              <div v-if="consultant.unReaded > 0" class="item">
                <el-badge :value="consultant.unReaded" class="item">
        ...
        >
          <template v-if="fromId !== user.uid">
            ...
            <div style="margin-right: 10px;margin-top: 20px">
              <div v-if="user.unReaded > 0" class="item">
                <el-badge :value="user.unReaded" class="item">
                  <el-icon size="25"><ChatDotRound /></el-icon>
                </el-badge>
              </div>
              <el-icon v-else size="25"><ChatDotRound /></el-icon>
